# Commercial License Agreement

## Animated Resume Template

This template is available for purchase and commercial use under the following terms:

### What You Can Do:
✅ Use the template for personal portfolio/resume
✅ Use the template for client projects (with Extended License)
✅ Modify and customize the template
✅ Create derivative works based on the template
✅ Use for commercial purposes (with appropriate license)

### What You Cannot Do:
❌ Resell or redistribute the template as-is
❌ Share the source code publicly
❌ Create competing template products
❌ Claim the template as your own creation
❌ Use without purchasing a license

### License Types:

#### Personal License ($19-29)
- Use for one personal portfolio/resume
- Modify and customize freely
- No attribution required
- Lifetime updates

#### Extended License ($49-79)
- Everything in Personal License
- Use for unlimited client projects
- Commercial use allowed
- Priority support

#### Developer License ($99-149)
- Everything in Extended License
- Use for multiple personal projects
- Remove "Built with" attribution
- Source code access
- Lifetime updates

### Support & Updates:
- Free updates for lifetime of product
- Email support for setup questions
- Bug fixes and improvements
- Documentation updates

### Refund Policy:
- 30-day money-back guarantee
- No questions asked
- Full refund if not satisfied

### Contact:
For licensing questions or custom licensing needs, contact:
[your-email@example.com]

---

© 2024 [Your Name/Company]. All rights reserved.
